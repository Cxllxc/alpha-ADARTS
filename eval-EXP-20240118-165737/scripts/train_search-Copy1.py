...
from model_searchrandowm import Network
from architect import Architect

parser = argparse.ArgumentParser("cifar")
parser.add_argument('--data', type=str, default='/root/Untitled Folder/data', help='location of the data corpus')
...
def main():
    ...
    if args.set == 'cifar100':
        train_data = dset.CIFAR100(root=args.data, train=False, download=False, transform=train_transform)
    else:
        train_data = dset.CIFAR10(root=args.data, train=False, download=False, transform=train_transform)
    ...
    for epoch in range(args.epochs):
        optimizer.step()  # 根据warning加入
        scheduler.step()
        lr = scheduler.get_last_lr()[0]  # 根据warning修改
...
def train2(train_queue, valid_queue, model, architect, criterion, optimizer, lr, epoch):
    objs = utils.AvgrageMeter()
    top1 = utils.AvgrageMeter()
    top5 = utils.AvgrageMeter()
